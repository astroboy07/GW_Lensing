...
class overlap_sie_basin():
    ...
    def F_source_sie(self, f):
        ...
        radius_data = np.array(df['source_x'])
        mu_1_data = np.array(df['mu_1'])
        mu_2_data = np.array(df['mu_2'])
        td_1_data = np.array(df['td_1'])
        td_2_data = np.array(df['td_2'])
        
        index_radius = np.where(radius_data == self.radius)
        mu_1 = mu_1_data[index_radius]
        mu_2 = mu_2_data[index_radius]
        td_1 = td_1_data[index_radius] * day
        td_2 = td_2_data[index_radius] * day
        '''
...
start = time.time()
#radius_range = my_lin(1.0e-4, 1.9e-4, 1)
bnds = [[-0.2, 0.2], [-np.pi, np.pi]]
overlap_optimized = overlap_sie_basin(params_source = initial_params_source, params_temp = initial_params_template)
#overlap_max = basinhopping(overlap_optimized.overlap, x0 = [0., 0.], seed = 42, disp = True, niter = 10)
overlap_max = dual_annealing(overlap_optimized.overlap, bounds = bnds, maxiter = 10)
print(overlap_max.fun, overlap_max.x[0], overlap_max.x[1])

end = time.time()
print(f'start time: {end - start}')

'''
def overlap_multiprocessed_lensing(radius_range, return_dict):

    bnds = [[-0.5, 0.5], [-np.pi, np.pi]]
    params_source = initial_params_source
    params_source['radius'] = radius_range
    overlap_optimized = overlap_sie_basin(params_source = params_source, params_temp = initial_params_template)
    overlap_max = basinhopping(overlap_optimized.overlap, x0 = [0., 0.], seed = 42, disp = True)
...
if __name__ == "__main__":
    ...
    #radius_range = my_lin(0.16, 1.0, 1)
    radius_range = np.linspace(0.16, 1.0, 1)
    ...
    w = csv.writer(open(datPath + "overlap_lensing_sie_theta=0_basinhopping.csv", "w"))
